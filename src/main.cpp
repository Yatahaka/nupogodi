    ...
    for (int row = 0; row < 5; row++) {
        for (int col = 0; col < 3; col++) {
            if (DIGITS[digit][row] & (1 << (2 - col))) {
                SDL_FRect px = { (float)(x + col * sz), (float)(y + row * sz),
                                 (float)sz, (float)sz };
    ...
    for (int dy = -rad; dy <= rad; dy++) {
        for (int dx = -rad; dx <= rad; dx++) {
            if (dx * dx + dy * dy <= rad * rad)
                SDL_RenderPoint(gRenderer, (float)(cx + dx), (float)(cy + dy));
        }
    }
}
...
static void drawApple(int x, int y) {
    ...
    SDL_RenderLine(gRenderer, cx, y + 4, cx + 6, y + 1);
}
...
static void drawBomb(int x, int y) {
    ...
    SDL_RenderLine(gRenderer, cx + 6, y + 4, cx + 10, y - 2);
    SDL_RenderLine(gRenderer, cx + 10, y - 2, cx + 8, y - 6);
...
static void drawBasket(int x) {
    ...
    for (int i = 1; i < 5; i++)
        SDL_RenderLine(gRenderer, x, BASK_Y + i * 4,
                       x + BASK_W, BASK_Y + i * 4);
    for (int i = 1; i < 10; i++)
        SDL_RenderLine(gRenderer, x + i * 10, BASK_Y,
...
static void doUpdate() {
    ...
    if (now - gs.lastSpawn >= (Uint64)SPAWN_DELAY) {
        gs.lastSpawn = now;
        FallingObj o;
    ...
    for (auto& o : gs.objs) {
        ...
        bool hitX = o.x + OBJ_SIZE > gs.baskX &&
                    o.x < gs.baskX + BASK_W;
        bool hitY = o.y + OBJ_SIZE > BASK_Y &&
                    o.y < BASK_Y + BASK_H;
...
int main() {
    ...
    gWindow = SDL_CreateWindow("Apple Catcher", WIN_W, WIN_H, 0);
