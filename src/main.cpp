...
// ── Tiny pixel font (3×5): digits 0–9 + colon ────────────────────────────────
...
struct Game {
    ...
    void update(const Uint8* keys) {
        ...
        for (auto& it : items) {
            ...
            SDL_Rect box{static_cast<int>(it.x), static_cast<int>(it.y), ITEM_SIZE, ITEM_SIZE};
    ...
    void draw(SDL_Renderer* r) const {
        ...
        for (int i = 0; i < MAX_LIVES; ++i)
            drawHeart(r, 12 + i*18, 38, 2, i < lives);
        ...
        if (flashing) {
            ...
            if (elapsed < 300) {
                ...
                SDL_SetRenderDrawColor(r, 220, 40, 40, static_cast<Uint8>(alpha * 255));
...
#ifdef __EMSCRIPTEN__
    // 0 fps = use requestAnimationFrame; 1 = simulate infinite loop
